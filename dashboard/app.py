...
def account_data() -> dict | None:
    ...
    return {
        ...
        "positions": [{
            "Symbol": str(p.symbol), "Qty": float(p.qty),
            "Entry Price": float(p.avg_entry_price), "Current Price": float(p.current_price),
            "Market Value": float(p.market_value), "P&L $": float(p.unrealized_pl),
...
def spy_returns(start: str, end: str) -> pd.Series:
    try:
        ...
        return yf.download("SPY", start=start, end=end, progress=False,
...
def _eq_df(logs: list[dict]) -> pd.DataFrame | None:
    rows = [{"date": l["timestamp"][:10], "equity": l["account_equity"]}
            for l in logs if "account_equity" in l]
    if len(rows) < 2:
        return None
...
""")
    ...
    c2.metric("Today P&L", f"${today_pnl:+,.2f}",
    ...
    if data["positions"]:
        ...
        with left:
            fig = px.pie(df_pos, values="Market Value", names="Symbol", title="Allocation", hole=0.4)
            fig.update_layout(margin=dict(t=40, b=0, l=0, r=0), showlegend=False)
            st.plotly_chart(fig, use_container_width=True)
        with right:
            st.subheader("Open Positions")
            d = df_pos.copy()
            for col in ("Entry Price", "Current Price"): d[col] = d[col].map("${:.2f}".format)
    ...
    if logs:
        with st.expander("Last run log"): st.json(logs[-1])
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if eq_df is None:
        st.info("Need at least 2 run logs to chart performance."); st.stop()

    st.plotly_chart(px.line(eq_df, y="equity", title="Equity Curve", labels={"equity": "USD"}),
                    use_container_width=True)

    c1, c2 = st.columns(2)
    with c1:
        st.plotly_chart(px.area(eq_df, y="drawdown", title="Drawdown",
    ...
    heat    = pd.DataFrame({"y": monthly.index.year, "m": monthly.index.month, "v": monthly.values})
    ...
    st.plotly_chart(px.imshow(pivot, color_continuous_scale="RdYlGn", text_auto=".1f",
    ...
    st.plotly_chart(px.line(pd.DataFrame({"Strategy": eq_df["cum_ret"], "SPY": spy_cum}),
                title="Cumulative Return vs SPY"), use_container_width=True)
    st.dataframe(pd.DataFrame({
        "Metric":   ["Total Return", "Sharpe (ann.)", "Max Drawdown"],
        "Strategy": [f"{eq_df['cum_ret'].iloc[-1]:.1%}", _sharpe(eq_df["ret"]),
                     _max_dd(eq_df["equity"])],
        "SPY":      [f"{spy_cum.iloc[-1]:.1%}" if len(spy_cum) else "—",
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.title("📋 Trade Log")
    rows = [{"date": l["timestamp"][:10], **order, "run_status": l.get("status")}
            for l in live_logs() for order in l.get("orders", [])]
    if not rows:
        st.info("No trades yet — run the bot to build history."); st.stop()
    ...
    if ticker != "All": mask &= df["symbol"] == ticker
    if side   != "All": mask &= df["side"]   == side
    df_f = df[mask]
    st.caption(f"{len(df_f):,} trades")
    st.dataframe(df_f, use_container_width=True, hide_index=True)
    st.download_button("⬇️ Download CSV", df_f.to_csv(index=False), "trade_log.csv", "text/csv")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not files:
        st.info("No backtest files in `results/`. "
                "Save an equity curve as `results/backtest_<name>.parquet`."); st.stop()

    chosen = st.selectbox("Backtest", [f.name for f in files])
    fpath  = next(f for f in files if f.name == chosen)
    try:
        df_bt = (pd.read_parquet(fpath) if fpath.suffix == ".parquet"
                 else pd.DataFrame(json.loads(fpath.read_text())))
    except Exception as e:
        st.error(f"Cannot load {chosen}: {e}"); st.stop()

    try: df_bt.index = pd.to_datetime(df_bt.index)
    except Exception: pass

    eq_col = next((c for c in ("equity", "portfolio_value", "value") if c in df_bt.columns), None)
    if eq_col:
        ...
        st.plotly_chart(px.line(df_bt, y=eq_col, title=f"Equity — {chosen}"),
                        use_container_width=True)
        c1, c2 = st.columns(2)
        with c1:
            st.plotly_chart(px.area(bt_dd.rename("drawdown"), title="Drawdown",
                    color_discrete_sequence=["#e74c3c"]), use_container_width=True)
        with c2:
            st.dataframe(pd.DataFrame({
                "Metric": ["Total Return", "Sharpe (ann.)", "Max Drawdown", "Rows"],
                "Value":  [f"{total:.1%}", _sharpe(bt_ret), f"{bt_dd.min():.1%}",
                           f"{len(df_bt):,}"],
            }), hide_index=True, use_container_width=True)
    else:
